...
function formatDuration(seconds) {
  ...
  for (let i = 0; i < timeUnits.length; i++) {
    ...
    if (seconds >= duration) {
      ...
      durationString += count === 1 ? `${count} ${unit}` : `${count} ${unit}s`;
    }
  }

  if (durationString.includes(", ")) {
    const lastCommaIndex = durationString.lastIndexOf(", ");
    durationString = durationString.substring(0, lastCommaIndex) + " and" + durationString.substring(lastCommaIndex + 1);
  }
...
console.log(formatDuration(60483));
